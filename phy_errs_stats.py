...
for data_f_name_list in filename_list : #data_fs :    
    ...
    for idx in xrange(0,len(correct_data_frames)-DATA_STRUCT_SIZE ,DATA_STRUCT_SIZE ):	
		...
		temp_errs.append(monitor_elem[tsf])
    ...
    phy_ =sum (map ( lambda x:x  , [i[1]  for i in temp_errs] ))
    phy_cck = sum (map ( lambda x:x  , [i[2]  for i in temp_errs] ))
    phy_ofdm= sum (map ( lambda x:x  , [i[3]  for i in temp_errs] ))
    Physical_errs[file_timestamp].append([phy_,phy_cck,phy_ofdm])
    del temp_errs
print Physical_errs
print "now processing the files to calculate time "

phy_err_time_series.sort(key=lambda x:x[0])
phy_err_cck_time_series.sort(key=lambda x:x[0])
phy_err_ofdm_time_series.sort(key=lambda x:x[0])
print "RECIVED FRAMES "

for i in range(0,len(err_time_series)):
    frame = err_time_series[i]
    print frame
